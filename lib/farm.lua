...
---one.
---
---Returns (aim_pos, hit_count, hits) where aim_pos = origin + best_dir * length
---(a point defining the cast line for issue_cast_position). (nil, 0, {}) when
---no unit is hittable.
...
function Farm.BestLineAim(origin, units, length, half_width, opts)
    ...
    if not best_dx then return nil, 0, {} end
    local aim_pos = Vector(ox + best_dx * length, oy + best_dy * length, oz)
    return aim_pos, best_n, best_hits
end
