...
describe("towers - registry: alive flag + measured hp-slope death eta", function()
    ...
    it("dead latch is permanent (towers never revive)", function()
        ...
        st = TW.Track(st, { { key = KEY, hp = 1800, alive = true } }, 104)
        assert_eq(TW.Alive(st, KEY), false)
        assert_eq(TW.DeathEta(st, KEY, 104), 0)
    end)
    it("stale sample disables the prediction", function()
        local st = TW.Track({}, { { key = KEY, hp = 1800, alive = true } }, 100)
        st = TW.Track(st, { { key = KEY, hp = 1600, alive = true } }, 102)
        assert_true(TW.DeathEta(st, KEY, 103) < math.huge, "fresh melt should predict")
        assert_eq(TW.DeathEta(st, KEY, 112), math.huge)
    end)
end)

describe("farm-route spine - load + smoke (lane/route/schedule/nav/map_data)", function()
    it("modules load and export their core API", function()
        ...
        assert_true(type(Lane.PredictMeeting) == "function" and type(Lane.ExpectedWave) == "function")
        assert_true(type(Route.Plan) == "function")
        assert_true(type(Schedule.Plan) == "function" and type(Schedule.NextWaveArrival) == "function")
        assert_true(type(Nav.SafeDest) == "function" and type(Nav.Ladder) == "function" and type(Nav.Stuck) == "function")
        assert_true(#MD.TOWERS >= 22 and #MD.CAMPS >= 18)
    end)
    it("PredictMeeting: mid symmetry -> midpoint", function()
        local Lane = require("lib.lane")
        local pm = Lane.PredictMeeting({ pos = { x = 0, y = 0 }, speed = 325 },
                                       { pos = { x = 1300, y = 0 }, speed = 325 })
        assert_true(pm ~= nil and math.abs(pm.point.x - 650) < 1, "midpoint expected")
        assert_near(pm.eta, 2.0, "gap/650 expected")
    end)
    it("NextWaveArrival: stamped phase beats the grid", function()
        local Schedule = require("lib.schedule")
        local nxt = Schedule.NextWaveArrival(100, 30, 14, 95)   -- stamp at 95 -> next at 125
        assert_near(nxt, 125, "stamped cadence expected")
    end)
end)
